...
| 8 | 서울시 생활권계획 시설(공원) 공간정보 Shapefile | 공원 영역·자연 환경 판정 | 공원 영역 주 데이터 | 1,888개 Polygon/MultiPolygon, 서울 bbox 100% | Edge 길이 중 Polygon 내부 비율 계산. 도보망 원본 공원·녹지 플래그를 대체하지 않음 | 아니오 |
...
| 8 | 서울시 생활권계획 시설(공원) 공간정보 Shapefile | 4개 sidecar가 함께 있고 1,888개 Polygon/MultiPolygon이 정상 판독됨. 서울 bbox 100%. 도보망 원본 공원·녹지 1,162개 중 직접 교차는 125개로 두 자료의 정의가 다름 | 공원 Polygon·자연 Score 후보 | `wired` | Polygon은 `nature_layer`에 보존하고 Edge 내부 길이 비율을 `park_overlap_ratio`에 저장. `raw_is_park_green`과 별도 근거로 유지 | `승인` | `ParkPolygonCollector → nature_layer → walk_edges.park_overlap_ratio` | E2, T, C | 미확인 |
...
현재 V1 서비스 데이터로 승인된 것은 기본 그래프로 사용하는 서울시 도보 네트워크와 자연 Layer 입력으로 사용하는 공원 Polygon이다. 공원 Polygon은 도보망 원본 `raw_is_park_green`을 대체하지 않고 `park_overlap_ratio`라는 별도 근거로 저장하며, `nature_score` 결합 정책은 추후 확정한다. 터널은 보행 불가 판정이 아니라 산책 쾌적도 감점 후보로 제한 사용한다. 횡단보도·육교 별도 RAW는 검증 근거만 보존하고 서비스 DB에는 적재하지 않는다. 지하도 별도 RAW는 V1 반영을 보류한다. 나머지 19개는 기존 분석 근거와 위 제안을 보고 채원이 사용·보류·참고 여부를 결정한다.
