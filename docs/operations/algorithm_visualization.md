...
`refinement_done`은 조립 계층이 호출한 같은 초기 후보의 개선 전후를 연결한다. 서로 다른 재시작의 첫 후보와 최종 승자를 임의로 묶지 않는다. `winner`는 이전 전체 최선과 새 전체 최선의 비교다. Local/VND 채택과 VNS 교란 후 판단은 실제 `RouteObjective` 비교값을 기록한다. 목표 범위 충족이 우선이며, 둘 다 범위 밖이면 거리 오차 → 재통행 → 선호 페널티, 둘 다 안이면 재통행 → 선호 페널티 → 거리 오차 순이다(#467, `RouteObjective.preference_penalty_ratio` 추가). 동률은 기존 경로를 유지한다.
