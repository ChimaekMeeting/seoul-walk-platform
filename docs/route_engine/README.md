...
- **세 지점 모두 이슈가 정한 "되돌아갈 기준"을 통과했다 — 단, 이 결론은 출발지 하나
  ...
  "밀도 티어 검증" 절을 함께 봐야 한다 — 저밀도(rural) 지역은 같은 기준에서 실제로
  걸린다.**
...
- **원인 규명(구축 단계가 급회전을 만드는지, ALNS repair가 급회전을 못 없애는지)과
  대응 여부는 이 관측의 범위 밖이다.** 여기서는 "저밀도 지역에서 게이트 통과율이
  실제로 떨어진다"는 사실 확인까지만 했다.
