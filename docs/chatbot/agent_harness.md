...
- API 출력: `POST /api/prewalk/init`은 `ChatResponse(status, thread_id, state)` JSON을 한 번에 반환한다. `POST /api/prewalk/intent`는 2026-09-25 후속부터 SSE(`text/event-stream`)로 바뀌었다 — `event: progress`(텍스트, Node 하나가 끝날 때마다), 마지막에 `event: result`(JSON, `ChatResponse`와 동일 스키마), 처리 중 실패 시 `event: error`(텍스트)를 내려보낸다(상세는 위 "파일 구조" 아래 "SSE 스트리밍 전환" 참고).
...
- **`PrewalkOrchestrator.orchestrator()`(async generator로 변경)**: `self.graph.ainvoke(state)` 한 번 호출하던 것을 `self.graph.astream(state, stream_mode="updates")`로 바꿨다. Node가 하나 끝날 때마다 `("progress", 문구)`를 `yield`하고(문구는 `prewalk_service.py`의 모듈 상수 `NODE_PROGRESS_MESSAGE`에서 Node 이름으로 찾음 — `extractor`/`weight_extractor`/`interviewer`/`route_executor` 4개 모두 등록됨), 그래프가 끝나면 `("result", ChatResponse)`를 `yield`하고 함수가 끝난다. 인증·소유권 실패 같은 조기 종료 지점도 전부 `yield "result", ChatResponse(status=...)` 뒤 `return`으로 바뀌었다(과거엔 그냥 `return ChatResponse(...)`).
...
- **실행 검증**:
  ...
  - **확인하지 못한 항목/남은 일**: 실제 FE와의 계약(이벤트 이름·포맷) 합의·문서 공유는 아직 안 했다. `scripts/test_prewalk_conversation.py`(수동 스크립트, `await orchestrator.orchestrator(...)`로 `ChatResponse`를 직접 기대)와 `tests/integration/check_circular_preference_artifact.py`(`TestClient`로 `/intent`를 호출하되 `response.json()`으로 검증)는 새 SSE 응답 형식에 맞춰 갱신하지 않았다 — 전자는 pytest 대상이 아니고, 후자는 실제 그래프 artifact가 있어야 실행되는 수동 스크립트라 이번 작업에서 실행 확인을 못 했다.
