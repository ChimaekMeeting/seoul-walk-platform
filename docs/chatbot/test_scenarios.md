...
> 상태: Current
> 기준일: 2026-08-20
> 관련 코드: `scripts/test_prewalk_conversation.py`, `src/agent/nodes/extractor.py`, `src/agent/nodes/interviewer.py`, `src/schema/prewalk_schema.py`, `src/interfaces/schema/prewalk_schema.py`(`ChatRequest.confirmation`)
> 검증 상태: 7차 실행(2026-08-03)까지 완료. **11개 시나리오 전부 확정.** 1번(의정부역)에서 `out_of_seoul` 경로가 정상 동작함을 확인했고, 6차 실행에서 발견한 문구 버그(circular 모드인데 "도착하고 싶으신 곳" 언급)를 `interviewer.py`에서 수정한 뒤 7차 실행으로 재확인 완료. **미해결 UX 이슈였던 3번**: 확인 대기 중 무관한 발화("오늘 날씨 어때?")에 재확인 질문이 그대로 반복될 뿐 아무 언급이 없던 문제 — 2026-08-03엔 원인만 파악하고 수정은 보류했으나, 2026-08-20 `Interviewer` 하드코딩 제거로 구조적 원인이 없어져 "호응 없이 명확히 선 긋기"로 재설계함(§아래 참고, 재실행 검증 전). **2026-08-20 이후 전체 재검증 필요**: `Interviewer`의 하드코딩 응답 문구(확인 질문·검색 실패·서울 밖 안내·fallback)를 전부 제거하고 `interview.yaml` LLM 생성으로 바꿔서, 1·2·3·7·8번이 거치는 코드 경로 자체가 달라졌다 — 아래 표의 "실행 결과"는 그 이전 코드 기준이라 최신 상태를 보장하지 않는다. §"2026-08-20" 절 참고. **2026-09-23 추가 변경으로 재검증 범위가 더 늘었다**(이 문서 범위인 `scripts/test_prewalk_conversation.py`로는 아직 재실행 못 함, 로컬 DB 없음 — `scripts/eval_interviewer.py`로만 간접 확인됨, `docs/chatbot/agent_harness.md` §9 "2026-09-23 후속" 참고): (a) `interview.yaml` 지침4(최종 확인)가 장소·거리를 재요약하지 않고 "이 코스로 진행할까요?"처럼 짧게만 확인하도록 바뀌어, 아래 6번처럼 "잠실역부터 강남역까지가 맞나요?" 형태의 확인 질문 예시는 더 이상 실제 문구와 다르다. (b) 지침0(무관한 주제 처리)에 "다리·도로 지명이어도 산책 판단이면 관련 있음" 조건이 추가돼, 2·3번 시나리오의 "무관한 발화" 판단 결과가 바뀔 수 있다. **2026-09-24/25 후속으로 확인 응답 메커니즘 자체가 바뀌어 재검증 범위가 더 늘었다**(마찬가지로 아직 이 스크립트로 재실행 못 함, `docs/chatbot/agent_harness.md` §9 "2026-09-24"·"2026-09-25 후속" 참고): (c) `ConfirmationClassifier`(자유 텍스트 긍정/부정 LLM 판정)가 삭제되고 `ChatRequest.confirmation`(`Optional[bool]`, FE 버튼 값) 필드로 대체됐다 — 4·5·6·10번의 "응" 턴은 이제 텍스트가 아니라 `confirmation=True`(버튼 클릭, `scripts/test_prewalk_conversation.py`의 `YES` sentinel)로 보낸다. (d) 3·9번처럼 확인 대기 중 버튼 없이 자유 텍스트만 보내는 경우는 이제 LLM 판정 없이 `confirmation`이 안 왔다는 사실만으로 결정론적으로 부정 처리된다 — 판단 로직이 바뀌었을 뿐 "부정으로 처리된다"는 기존 예상 결과 자체는 유지된다. (e) 11번("그걸로 해줘"가 긍정으로 판정되는지)은 **더 이상 성립하지 않는 시나리오**다 — `confirmation` 필드 없이는 어떤 자유 텍스트도 확인으로 인정되지 않으므로, 스크립트에서도 "자연어만으로는 절대 확정되지 않아야 한다"는 반대 방향 검증으로 목적을 바꿨다(§아래 실행 결과는 옛 시나리오 기준이라 더는 유효하지 않음). `POST /api/prewalk/intent` 응답 형식도 JSON 한 번에서 SSE로 바뀌었으나(2026-09-25 후속), 이건 스크립트 내부 파싱 방식만 바뀐 거라 시나리오 자체의 기대 결과에는 영향이 없다.
...
- `WeatherChecker`와 `RouteExecutor`(경로 엔진·profile 선택)는 이 문서의 범위가 아니다. 챗봇 대화 흐름(`Extractor`·`Interviewer`)과 직결되는 부분만 다룬다. `ConfirmationClassifier`는 2026-09-24에 제거됐다 — 아래 표의 해당 카테고리 라벨·시나리오는 그 시점 기준 기록이다(§아래 "2026-09-24/25 후속" 참고).
