...
- _update_shortest_km(state): oneway_shortest·oneway_random 두 모드 모두에서 위치가
  확정되면 state.shortest_km을 A*로 채우고, 그 외에는 None으로 지운다(stale 방지).
- _is_oneway_shortest_conflict(state): oneway_random에서 목표 거리가 state.shortest_km
  이하면(우회할 여지가 없는 요청) True.

route_service.get_shortest_km을 mock해 실제 그래프·A* 없이 결정론적으로 확인한다
(scripts/eval_interviewer.py는 이 판단 이후 실제로 생성되는 안내 문구의 품질을 실제
OpenAI 호출로 따로 검증한다 — 이 파일의 범위가 아니다).
"""
import sys
import importlib
...
def _make_interviewer(shortest_km):
    """route_service.get_shortest_km이 고정값을 반환하도록 mock한 Interviewer를 만든다."""
    route_service = MagicMock()
    route_service.get_shortest_km.return_value = shortest_km
    return Interviewer(route_service=route_service), route_service
...
def test_oneway_shortest_mode_populates_field_but_never_conflicts():
    ...
    최단거리는 똑같이 state.shortest_km에 채워져야 한다(2026-09-23 신규 요구사항)."""
