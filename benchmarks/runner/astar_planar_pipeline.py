"""좌표 -> 경유지 후보 풀 -> Beam 개선 -> Dijkstra/A*(Haversine)/A*(Planar)/
A*(Avoid) 경로 비교 파이프라인.

waypoint_pool.py / waypoint_beam.py / landmark_planar.py / landmark_avoid.py를
독립 모듈 그대로 이어 붙인 수동 검증 스크립트다. route_service.py/
WaypointComposerEngine 등 프로덕션에는 연결하지 않았고 engines/__init__.py에도
등록하지 않는다.
...
네 방식(Dijkstra/Haversine/Planar/Avoid)의 총 거리는 admissible한 휴리스틱이면
항상 동일해야 한다(cost_mismatch로 확인). 실제 경로가 얼마나 겹치는지는
`pairwise`의 엣지 집합 Jaccard 유사도와 완전 일치 구간 수로 측정한다.

Planar/Avoid 랜드마크 표는 전체 실행에서 1회만 준비하고 모든
--target-km/--repeats에서 재사용한다. Avoid의 랜드마크 개수는 Planar가 실제로
뽑은 개수에 맞춘다(둘 다 ALT 방식이라 같은 랜드마크 수로 비교해야 공정하다).
"""
...
_METHODS = ["dijkstra", "haversine", "planar", "avoid"]
...
def _leg_path(graph, method, heuristics, weight_fn, a, b):
    if method == "dijkstra":
        return nx.shortest_path(graph, a, b, weight=weight_fn, method="dijkstra")
    return nx.astar_path(graph, a, b, heuristic=heuristics[method], weight=weight_fn)


def _run_query(
    *, graph, utils, weight_fn, heuristics, p1, target_km, args, parser
):
    ...
    # 3) 구간(leg)마다 네 방식(Dijkstra/Haversine/Planar/Avoid) 경로를 모두 복원해 비교
    stops = (p1, *best.waypoint_ids, p1)
    seconds = {m: 0.0 for m in _METHODS}
    total_m = {m: 0.0 for m in _METHODS}
    edge_sets_per_leg: list[dict[str, tuple[set, list[int]]]] = []

    for a, b in zip(stops, stops[1:]):
        leg_paths: dict[str, tuple[set, list[int]]] = {}
        for m in _METHODS:
            t0 = perf_counter()
            try:
                path = _leg_path(graph, m, heuristics, weight_fn, a, b)
            except nx.NetworkXNoPath:
                parser.error(f"구간 {a} -> {b} 사이에 연결된 경로가 없습니다({m}).")
            seconds[m] += perf_counter() - t0
            total_m[m] += utils.calc_distance(path)
            leg_paths[m] = (_edge_set(path), path)
        edge_sets_per_leg.append(leg_paths)

    legs = len(stops) - 1
    cost_mismatch = any(abs(total_m[m] - best.distance_m) > 1e-6 for m in _METHODS)

    pairwise: dict[str, dict] = {}
    for m1, m2 in itertools.combinations(_METHODS, 2):
        jaccards = []
        exact = 0
        for leg_paths in edge_sets_per_leg:
            e1, p1_path = leg_paths[m1]
            e2, p2_path = leg_paths[m2]
            jaccards.append(_jaccard(e1, e2))
            exact += p1_path == p2_path
        pairwise[f"{m1}_vs_{m2}"] = dict(
            exact_legs=exact,
            mean_jaccard=sum(jaccards) / legs,
        )
...
def main():
    ...
    t0 = perf_counter()
    planar_landmarks = select_landmarks_planar(graph, args.n_sectors)
    planar_heuristic, _ = build_alt_heuristic(graph, planar_landmarks, weight="length")
    planar_setup_seconds = perf_counter() - t0

    t0 = perf_counter()
    avoid_landmarks = select_landmarks_avoid(graph, len(planar_landmarks), seed=args.avoid_seed)
    avoid_heuristic, _ = build_alt_heuristic(graph, avoid_landmarks, weight="length")
    avoid_setup_seconds = perf_counter() - t0

    heuristics = dict(haversine=haversine_heuristic, planar=planar_heuristic, avoid=avoid_heuristic)
    ...
    print(json.dumps(dict(
        ...
        n_landmarks_planar=len(planar_landmarks),
        n_landmarks_avoid=len(avoid_landmarks),
        planar_setup_seconds=planar_setup_seconds,
        avoid_setup_seconds=avoid_setup_seconds,
    )))

    for target_km in args.target_km:
        for repeat in range(args.repeats):
            result = _run_query(
                graph=graph, utils=utils, weight_fn=weight_fn, heuristics=heuristics,
