...
⚠ 기준값이 오염돼 있다(2026-09-17 실측). circularity_q는 자기 교차 경로에서 신발끈 부호
상쇄로 면적을 최대 32배 과소평가한다(8자 실측: 두 고리 +149,900/-140,864 m², 상쇄 96.9%,
Q 0.0045 대 부호무시 0.1463). 그 경로의 repeated_edge_ratio는 0.000이라 "재통행이
걸러낸다"는 기존 면책도 성립하지 않는다(benchmarks/results.py::circularity_q 참고).
따라서 이 스크립트의 상관 수치를 목적함수 승격이나 임계값 확정의 근거로 쓰면 안 된다.
원형성 자체가 게이트·순위·목적함수 어디에도 쓰이지 않기로 정해졌으므로(2026-09-17,
aggregate_results.py 모듈 docstring "순위 규칙" 참고) 이 스크립트는 관측용으로만 남긴다.
...
def verdict(correlation_df: pd.DataFrame) -> str:
    ...
    return (
        f"[결론] 기준값(circularity_q)을 가장 잘 예측하는 대리 지표: {best['지표']} "
        f"(Spearman {best['spearman']:.3f}, 95%CI [{best['95%CI_low']:.3f}, {best['95%CI_high']:.3f}], "
        f"n={int(best['n'])}).\n"
        "        ⚠ 이 결과를 목적함수 승격 판단(이슈 I)의 근거로 쓰지 마세요. 기준값이 "
        "자기 교차 경로에서\n"
        "        최대 32배 과소평가되는 값이라(2026-09-17 실측, circularity_q docstring 참고) "
        "상관 자체가 오염돼 있습니다.\n"
        "        Spearman은 단조 관계만 잡으므로 역U자 관계도 놓칩니다 — 실제로 각도 지표는 "
        "N=4에서 r=0.002였지만\n"
        "        구간별로 보면 이상값(36도) 부근이 가장 원형에 가까웠습니다."
    )
