"""
benchmarks/solvers/bi_dijkstra_solver.py

src/route_engine/engines/oneway_bi_dijkstra.py (OnewayBidirectionalDijkstraEngine)를
BasePathSolver 규격으로 감싸는 어댑터. Dijkstra(dijkstra_solver.py)·A*(astar_solver.py)와
같은 입력으로 나란히 비교하기 위한 baseline.
"""
...
from src.route_engine.engines.oneway_bi_dijkstra import OnewayBidirectionalDijkstraEngine
...
class OnewayBidirectionalDijkstraSolver(BasePathSolver):
    def __init__(self, name: str = "Bidirectional Dijkstra(oneway)"):
        super().__init__(name)
    ...
    def solve(self, graph, start_node, target_node, params: dict) -> dict:
        ...
        engine = OnewayBidirectionalDijkstraEngine(
            inp=inp,
            G=graph,
