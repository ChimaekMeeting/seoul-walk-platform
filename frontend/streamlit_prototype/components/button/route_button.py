...
class RouteButton:
    ...
    def _build_context(self, selected_mode: str, distance_km: float, lat: float, lng: float) -> dict:
        """
                ...
                if requires_destination(selected_mode) and not st.session_state.end:
                    ...
                    with st.spinner("최적의 경로를 계산하는 중..."):
                        ...
                        if is_flat_mode(selected_mode):
                            result = run_flat_route(self.G, selected_mode, st.session_state.start, st.session_state.end, distance_km)
                            if result:
                                self._save(result)
                                st.rerun()
                        else:
