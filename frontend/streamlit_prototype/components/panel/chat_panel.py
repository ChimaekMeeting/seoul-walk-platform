...
class ChatPanel:
    ...
    async def chat_and_assign_weights(self, prompt: str):
        ...
        if state.get("is_complete"):
            st.session_state.weights = None # removed old flow
            route_res = state.get("route_result")
            if route_res:
                if route_res.get("status") == "FAILED":
                    st.session_state.route_error_message = (
                        f"경로 생성 실패: {route_res.get('fallback_reason')}"
                    )
                else:
                    st.session_state.route_error_message = None
                    st.session_state.route_result = route_res
                    st.session_state.route_coordinates = route_res.get("coordinates")
                    st.session_state.route_distance = route_res.get("total_km")
    ...
    def render(
        ...
        if state and state.get("is_complete"):
            ...
            # mode_map 반환 로직 완전 제거 (프론트는 렌더러 역할)
            return None
        return None
