...
# set_page_config()는 반드시 첫 번째 Streamlit 명령이어야 함
...
print(f"dotenv: {time.time()-t:.2f}s")
t = time.time()
...
print(f"html gps: {time.time()-t:.2f}s")
t = time.time()
...
print(f"query_params: {time.time()-t:.2f}s")
t = time.time()
...
print(f"weather: {time.time()-t:.2f}s")
t = time.time()
...
if input_mode == "직접 설정":
    selected_mode_label = st.sidebar.selectbox(
        "경로 모드", options=list(mode_options.keys()), key="route_mode_select"
    )
    selected_mode = mode_options[selected_mode_label]
    distance_km = st.sidebar.slider(
        "목표 거리 (km)", 1.0, 10.0, 3.0, 0.5, key="distance_km"
    )
    ...
    slope_w = st.sidebar.slider(
        "경사 회피 (평지 선호)", 0.1, 2.0, 1.0, 0.1, key="slope_w"
    )
else:
...
if input_mode == "AI 챗봇":
    ...
    if state and state.get("next_node") == "end":
        st.success(
            "📍 산책 정보 수집 완료! 아래 지도에서 출발지를 확인하고 경로를 추천받으세요."
        )
        ...
        if user_context:
            mode_map = {
                ...
                "Distance": "oneway_random",
            }
            selected_mode = mode_map.get(
                user_context.get("mode", "Circular"), "circular"
            )
...
if st.session_state.get("route_result"):
    ...
    with col_res2:
        mode_label = get_mode_label(result["mode"])
...
if input_mode == "직접 설정" and st.session_state.start:
    ...
    if st.button("🚶 경로 추천받기", type="primary", use_container_width=True):
        if requires_destination(selected_mode) and not st.session_state.end:
            ...
            with st.spinner("최적의 경로를 계산하는 중..."):
                ...
                # 평지 전용 모드 분기
                if is_flat_mode(selected_mode):
                    result = run_flat_route(
                        G,
                        selected_mode,
                        st.session_state.start,
                        st.session_state.end,
    ...
    if (
        ...
        and not st.session_state.route_coordinates
    ):
        user_context = state.get("user_context", {})
