...
**더 중요한 발견**: 세 엔진의 95% 신뢰구간이 서로 완전히 겹치고, 심지어 결정적 엔진들(`beam-wp-local` 789.3, `beam-wp-vnd` 792.3, `beam-wp` 797.5, `grasp-wp-local` 800.7, `grasp-wp-vnd` 803.3)의 값도 대부분 이 신뢰구간 안에 들어온다.

**정정(추가 검증)**: 처음에는 이 정황만 보고 "`grasp-wp-vns`(740.7)만 신뢰구간 밖이라 확실히 다르다"고 결론 내렸었다. 그런데 `grasp-wp-vns`도 결정적 엔진 6종과 마찬가지로 seed=42 단일 실행(25개 시나리오, n=25)이라 **자기 자신의 신뢰구간을 낸 적이 없었다** — 그래서 25개 시나리오를 표본으로 CI를 계산했더니 [676.8, 804.7]로 오히려 가장 넓고, 다른 엔진들 대부분과 겹쳤다. 나아가 9종 모두 **같은 25개 시나리오를 공유**하므로 평균끼리 비교(unpaired)하는 대신 **시나리오별로 짝지어(paired)** `grasp-wp-vns`가 각 엔진을 실제로 이긴 비율을 세어보니 **52~64%(동전 던지기 수준)**에 그쳤다 — 낮은 평균은 "항상 조금씩 이겨서"가 아니라 "일부 시나리오에서 크게 이겨서 평균이 끌려 내려간 것"이었다. **따라서 "9종 중 grasp-wp-vns만 확실히 다르다"는 결론은 정정한다: 9종 모두 거리당 회전량 기준으로는 서로 확실하게 우열을 가리기 어렵다.** 상세 표와 짝지은 비교 수치는 [FINAL_REPORT.md](FINAL_REPORT.md) §5 참고.
...
1. **주 분포 표의 6종(결정적 엔진)은 각 엔진 1회 실행(seed=42 고정) 결과다.** `grasp-wp-alns`/`beam-wp-alns`/`beam-wp-vns` 3종은 seed 반복 측정(10개 시드 × 25개 시나리오, n=250)으로 보정했다 — 아래 "seed 반복 측정" 절 참고. 짝지은(paired) 시나리오별 비교까지 반영하면, 9종 모두 거리당 회전량 기준으로는 서로 확실하게 우열을 가리기 어렵다(`grasp-wp-vns`의 낮은 평균도 다른 엔진을 이기는 비율이 52~64%에 그쳐 확정적이지 않음 — 위 "정정" 참고).
...
- [완료] `grasp-wp-alns`/`beam-wp-alns`/`beam-wp-vns` 25개 시나리오 × 10개 시드 재측정 — `beam-wp-vns`가 최하위(845.4)에서 중위권(794.6)으로 순위가 뒤집힘(위 "seed 반복 측정" 절 참고)
- [완료] 짝지은(paired) 시나리오별 비교로 재검증 — `grasp-wp-vns`도 다른 8종을 이기는 비율이 52~64%뿐이라 "9종 중 유일하게 확실히 다르다"는 이전 결론을 정정, 9종 모두 통계적으로 뚜렷이 구분되지 않음(위 "정정(추가 검증)" 참고)
