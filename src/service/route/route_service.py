...
from src.service.route.path_oneway_random import oneway_random_route # 신규 모듈 임포트
import time
...
def get_route(context: dict, weights: dict, G_full: nx.Graph = None) -> dict:
    ...
    print(f"[1] load_graph_near: {time.time()-t0:.2f}s") # 프린트 유지
    t1 = time.time()
    ...
    print(f"[3] route algorithm: {time.time()-t2:.2f}s") # 프린트 유지
    t3 = time.time()
    ...
    print(f"[4] prune+extract: {time.time()-t3:.2f}s") # 프린트 유지
    print(f"[total] {time.time()-t0:.2f}s") # 프린트 유지
