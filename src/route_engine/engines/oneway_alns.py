...
class OnewayAlnsEngine:
    ...
    def run(self) -> List[WalkRouteResponse]:
        ...
        우회 편도 경로를 생성합니다.
        """
        logger.info(f"랜덤 편도 ALNS 경로 생성 엔진을 시작합니다: target_km={self.inp.target_km}, scoring_mode={self.scoring_mode}, weights={self.weights}")
    ...
    def find_path(self, start: int, end: int, target_km: float = 3.0) -> list[int]:
        """
        ALNS 기반 우회 편도 경로를 생성합니다.
        """
        target_m = target_km * 1000  # 목표 거리를 미터로 환산함
        rng = random.Random(self.seed)  # 엔진 전용 난수 인스턴스 → 재현성 보장

        # custom_score 기준 최단경로 — 초기 해 구성 실패/불가 시의 최종 대체 경로로도 사용함
        # destroy/repair 재연결(_repair_shortest/_repair_detour)에서도 재사용하도록 캐싱함
        self._min_ratio = self.utils.min_cost_length_ratio(self.G)
        try:
            base_shortest = self.utils.astar_path(start, end, weight="custom_score", min_ratio=self._min_ratio)
        except nx.NetworkXNoPath:
            logger.warning("출발-도착 간 경로가 존재하지 않습니다.")
            return []

        # 1단계: 출발지 → 중간지점 (탐욕 구성)
        cost, nodes, dist, visited = self._find_start_to_waypoint(start, end, target_m)

        # 2단계: 중간지점 → 도착지 (도착 연결) → 초기 완성 해
        initial = self._find_waypoint_to_end(nodes, visited, end)
        if initial is None:
            logger.warning("ALNS 초기 해 구성에 실패해 최단 경로로 대체합니다.")
            return base_shortest

        # 3단계: destroy/repair 반복 개선
        best_path, best_key = self._improve(initial, end, target_m, rng)

        logger.info("ALNS 편도 경로 선택: 노드=%d개, 거리초과=%.0fm, 품질밀도=%.3f",
    ...
    def _find_waypoint_to_end(self, nodes: list[int], visited: set, end: int):
        """
        2단계: 중간지점 → 도착지 경로를 생성합니다.
        """
        return self.utils.connect_to(nodes, visited, end)

    def _improve(self, initial: list[int], end: int, target_m: float, rng: random.Random) -> tuple:
        """
        3단계: destroy(구간 제거) + repair(재연결)를 반복하며 경로를 개선합니다.
        """
        destroy_ops = [self._destroy_random, self._destroy_worst]
        repair_ops  = [self._repair_shortest, self._repair_detour]
        ...
        for _ in range(_ALNS_ITERS):
            di = self._roulette(d_w, rng)
            ri = self._roulette(r_w, rng)
    ...
    def _destroy_random(self, path: list[int], rng: random.Random):
        ...
        seg = rng.randint(1, min(_SEG_MAX, len(path) - 1 - i))
        return i, i + seg - 1
