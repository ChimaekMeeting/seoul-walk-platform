...
class OnewayRcspEngine:
    ...
    def find_path(self, start: int, end: int, target_km: float = 3.0) -> list[int]:
        ...
        min_ratio = self._min_cost_per_m()  # end 방향 유도용 비용 하한 비율(A* 휴리스틱 스케일에도 재사용)
    ...
    def _min_cost_per_m(self) -> float:
        ...
        직선거리(m) × 이 값은 end 방향 유도용 비용 하한 추정치로 사용됩니다(A* 휴리스틱과 공유).
        """
        return self.utils.min_cost_length_ratio(self.G)
