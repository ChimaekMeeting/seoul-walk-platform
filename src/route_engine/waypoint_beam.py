...
def beam_search(
    *,
    ...
    rank_penalty(prev_id, next_id)는 랭킹 전용 보정항이다(2026-09-06, "Beam 방향
    다양성 랭킹" 추가 — GRASP의 _rank_next_waypoint_candidates가 쓰는
    "score = 거리오차 + 방향다양성 페널티" 구조와 동일하게, 페널티를 실제 거리
    (partial_m/closed_m, 따라서 WaypointOrder.distance_m/error_m)와 분리해서 관리한다
    — 목표거리 비교는 항상 순수 실거리 기준으로 유지된다. 각 확장 단계에서 정확히
    한 번(직전 경유지→다음 후보) 호출되며, 도착지로 닫는 임시 연결(next_id→end_id)에는
    적용되지 않는다 — GRASP도 폐합 구간에는 방향 페널티를 적용하지 않는다. 호출부가
    첫 경유지 선택(prev_id == start_id)에 페널티를 줄지 말지는 콜러블 자체가 정한다.
    tolerance_ratio와 함께 쓸 수 없다 — WaypointObjective.quality()가 재통행 모드에서
    반환하는 첫 성분은 무차원 비율(overlap)이라, 미터 단위인 페널티를 그대로 더하면
    단위가 어긋난다. 이 훅은 아직 waypoint_alns.py::alns_search()에는 없다(의도적 —
    두 함수의 계약을 항상 동시에 맞출 필요는 없다는 판단, 2026-09-06)."""
