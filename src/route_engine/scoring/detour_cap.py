...
팀 검토용 실험: 가중 경로의 우회 거리를 제한하는 순수 함수(#445).

일반 요청에는 연결하지 않는다. 30%나 최단 강제 대체는 합의된 서비스 정책이
아니며, 팀 검토 후 재사용할 수 있도록 구현·테스트만 보존한다.
논의 항목: docs/proposals/route_engine_detour_policy_proposal.md

왜 엔진 안이 아니라 별도 함수인가
---------------------------------
1. OnewayAstarEngine.find_path()는 벤치마크 solver가 직접 호출한다. 엔진 내부에서
   상한을 적용하면 solver가 재는 값이 조용히 달라진다.
2. 이 실험은 **요청 전체**를 비교한다. 각 구간에 같은 비율을 적용해도 합계의 비율이
   구간 수만큼 증가하지는 않는다. 전체 비교는 구간 간 우회량 배분을 허용하기 위한 선택이다.
